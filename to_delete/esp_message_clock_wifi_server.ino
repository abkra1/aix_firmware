...
class SingleMessage {
  ...
  public:
    ...
    void Init(String messageIn, size_t colIndex, size_t maxLoopsIn) {
      ...
      maxCalls = (maxPos+LED_COLS+1) * maxLoopsIn;
      actPos = LED_COLS;
      numCalls = 0;
...
String makeHTMLPage()
{
    ...
    String theValue = String("<!DOCTYPE html><html><body><h2>AX-Clock<h2><h3> Wifi Credentials</h3>")
...
void loop() {
  ...
  {  // as init show all colors and pixels for 2 secs
     ...
     if (configMode) {
       ...
       matrix.setTextColor(matrix.Color(Colors[COL_WHITE][COL_R], Colors[COL_WHITE][COL_G], Colors[COL_WHITE][COL_B]));
       matrix.print(brand + clockVersion);
     }
     matrix.show(); 
     printf("done show\n");
